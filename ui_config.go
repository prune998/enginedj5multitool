...
func (t *ConfigTool) View(a *App) {
	a.L("Settings", FontSize(18), FontWeight(WeightBold))
	t.ensureLoaded()

	a.L(fmt.Sprintf("config file: %s", t.path), FontSize(11), TextColorVec(a.pal().textDim))

	Container(Attrs(Expand, Gap(8), Pad2(6, 0)), func() {
		// Library
		Container(Attrs(Gap(2)), func() {
			a.L("Default Engine DJ database", FontSize(11), TextColorVec(a.pal().textDim))
		...
		// Theme
		Container(Attrs(Gap(2)), func() {
			a.L("Theme", FontSize(11), TextColorVec(a.pal().textDim))
			theme := &t.draft.Theme
		...
		// Browser width
		Container(Attrs(Gap(2)), func() {
			a.L("Track list width at startup", FontSize(11), TextColorVec(a.pal().textDim))
